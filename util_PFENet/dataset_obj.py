...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, exclude_list=None):    
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, transform=None, mode='train', use_coco=False, use_split_coco=False):
        ...
        if not use_coco:
            ...
            if self.split == 3: 
                ...
                #self.sub_val_list = list(range(6, 21))
        ...
        if self.mode == 'train':
            self.data_list, self.sub_class_file_list = make_dataset(split, data_root, data_list, self.sub_list, exclude_list=self.sub_val_list)
    ...
    def __getitem__(self, index):
        ...
        target_pix_list = []
        for class_chosen in label_class:
            target_pix_list.append(np.where(label == class_chosen))
