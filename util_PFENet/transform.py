...
class Resize_fixSize(object):
    ...
    def __call__(self, image, label):
        image_resize = cv2.resize(image, dsize=(self.size, self.size), interpolation=cv2.INTER_LINEAR)
        label_resize = cv2.resize(label.astype(np.float32), dsize=(self.size, self.size),interpolation=cv2.INTER_NEAREST)
